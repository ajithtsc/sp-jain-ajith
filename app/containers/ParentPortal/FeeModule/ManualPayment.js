...
export default function ManualPaymentPage({ setLoading, setValue }) {
  ...
  const { date, ref } = Yup;
  const [fileToBase64data, setFileToBase64] = useState()
  ...
  const validationSchema = Yup.object().shape({
    ...
    .test('file-type', 'Only PDF, JPG, PNG, JPEG are Allowed', (value) => {
  ...
  const handleChangeFile = async (e) => {
    ...
    const updateManualPaymentData = () => {
      setManualPymentData({
        ...manualPymentData,
        [name]: file,
        fileDetails: file,
      });
      setFileToBase64(FileToBase64(file))
    };
        ...
        const attachments = [
          {
            ...
            body: fileToBase64data[0].split(',')[1],
