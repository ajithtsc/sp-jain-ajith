...
const AddRows = props => {
  ...
  const [totalAverageError, setTotalAverageError] = useState('');
  ...
  const handleChange = (e, index, i) => {
    ...
    if (name === 'Sub_Component_Weightage__c') {
      if (filterVal === '100') {
        setTotalAverageError('');
      } else {
        setTotalAverageError(
          `${filterVal}% (Percentage is not adding up to 100%)`
        );
      }
    }

    const isChecked = dataContent[i]['Course_Subcomponents__r']['records'].some(
      item =>
        item.Integrate_with_Black_Board__c || item.Integrate_with_Mettle__c
    );
  ...
  const handleSaveClick = i => {
    ...
    let activityError = checkActivity(
      data[i]['Course_Subcomponents__r']['records']
    );
    payLoad.forEach(d => {
      totalWeightage += Number(d.sub_weightage);
    });

    let totalAverageError = '';
    if (totalWeightage !== 100) {
      totalAverageError = 'Total weightage is not equal to 100%';
    }

    setActivityError(activityError);
    setTotalAverageError(totalAverageError);

    const isChecked = data[i]['Course_Subcomponents__r']['records'].some(
      item =>
        item.Integrate_with_Black_Board__c || item.Integrate_with_Mettle__c
    );
    ...
    if (payLoad.length > 0 && !activityError && !totalAverageError) {
      setLoading(true);
                              ...
                              <>
                                <Button
                                  ...
                                  onClick={event => {
                                    ...
                                    let arr = [...data];
                                    let totalAverageErrorMsg = '';
                                    let activityErrorMsg = '';

                                    let totalAverageMessage = handleTotalAverage(
                                      x?.Course_Subcomponents__r?.records
                                    );
                                    if (
                                      totalAverageMessage.includes(
                                        'Percentage is not adding up to 100%'
                                      )
                                    ) {
                                      totalAverageErrorMsg =
                                    ...
                                    if (
                                      ...
                                      setActivityError(activityErrorMsg);
                                    } else {
                                      setTotalAverageError('');
                                      setActivityError('');
                                      let openDialog =
                                    ...
                                    <KenCheckbox
                                      ...
                                      onChange={e => {
                                        ...
                                        if (e.target.checked) {
                                          ...
                                          const isAnyCheckboxSelected = newArr[
                                            i
                                          ]['Course_Subcomponents__r'][
                                            'records'
                                          ].some(
                                            item =>
                                              item.Integrate_with_Black_Board__c ||
                                              item.Integrate_with_Mettle__c
                                          );
